// micro-wealth-builder/api/login.js
...
export default async function handler(req, res) {
  ...
  const allowedEmail = process.env.USER_EMAIL
  const passHash = process.env.USER_PASS_HASH
  ...
  const token = signJwt({ sub: email, role: 'user' }, 86400)
